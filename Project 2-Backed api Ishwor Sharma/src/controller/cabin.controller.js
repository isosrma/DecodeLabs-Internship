...
export const getCabin = async (req, res) => {
    try {
        ...
        if (typeof maxCapacity === "string" && maxCapacity.trim() !== "") {
            const [minCap, maxCap] = maxCapacity.split("-").map(Number);

            if (Number.isFinite(minCap) && Number.isFinite(maxCap)) {
                where.maxCapacity = { gte: minCap, lte: maxCap };
            } else {
                const exactCapacity = Number(maxCapacity);
                if (Number.isFinite(exactCapacity)) {
                    where.maxCapacity = exactCapacity;
                }
            }
        }

        const allowedSortFields = ["createdAt", "regularPrice", "name"];
        ...
        const totalCabins = await prisma.cabin.count({ where });

        res.json({
            page,
            totalPages: Math.ceil(totalCabins / limit) || 1,
            totalCabins,
            cabins,
        });
    } catch (error) {
